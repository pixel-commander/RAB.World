...
WORKING COPY AND CAPTURE
The current settings.json links the local original ZIP and separate working copy.
Use capture-change baseline once, then begin/finish around every discrete edit.
Evidence objects preserve file bytes once by hash, with no ZIP copied per round.
Read the capture-change tool README before editing.
