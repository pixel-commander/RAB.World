...
Use capture-change begin/finish around every discrete code edit. changes/ holds
focused records; source_changes in summary.json links those records. Exact file
versions live once in the session evidence store. Training examples should select
relevant blocks with sufficient context, not automatically include every file.

LESSON PREPARATION
Fill lesson.template.json and its review sidecar into lessons/ after reviewing the
change. Keep code blocks focused and explanations checkable. Read the session
PROCESS.txt; evaluation cases and press records belong at the session level.
