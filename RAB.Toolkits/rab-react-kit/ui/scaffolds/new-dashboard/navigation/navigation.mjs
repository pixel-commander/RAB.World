...
export const run = async ({ options, helpers }) => {
  ...
  const result=await helpers.runReactComponentStamp({templateUrl:new URL('../template/navigation.tsx',import.meta.url),parentOption:'location',templateValues:{NAV_ROWS:JSON.stringify(rows,null,2),NAV_LABEL:JSON.stringify(options.label??'Main navigation'),NAV_CLASS:options.name.replace(/([a-z0-9])([A-Z])/g,'$1-$2').toLowerCase()}});
