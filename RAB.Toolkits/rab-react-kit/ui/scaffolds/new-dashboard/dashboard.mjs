...
import { run as createNavigation } from './navigation/navigation.mjs';
...
export const run=async({options,helpers})=>{
  ...
  const page=await helpers.runTool({key:'new-page',options:{name:options.name,location:pageLocation}});
  const grid=await helpers.runTool({key:'react/apply/grid-layout',options:{file:page.result.file,layout}});
  let navCreated=true;
  try{await createNavigation({options:{name:navName,location:componentLocation,rows:[{id:options.name,name:options.name,path:'/'}]},helpers});}catch(error){if(error.code!=='EEXIST')throw error;navCreated=false;}
