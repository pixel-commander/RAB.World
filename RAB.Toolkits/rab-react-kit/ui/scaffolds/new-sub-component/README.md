...
This scaffold creates a child React component inside the required parent_path folder with native div attributes and an optional canonical grid. Conversation input asks for the name, grid and container class. An empty grid or `none` skips the grid; an empty class skips skin. Optional display title, description, indexed and save_as_text inputs preserve supplied values.
